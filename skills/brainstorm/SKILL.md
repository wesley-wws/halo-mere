---
name: brainstorm
description: Use when the user is actively exploring a problem, requirement, or design decision and has not yet committed to a direction. English triggers include "help me think through X", "I'm not sure which approach to take", "what are my options for Y", "I can't decide between A and B", "how should I approach Z". 中文触发例句包括「帮我想一下」「我拿不定主意」「有哪些思路」「A 和 B 选哪个」「这事该怎么下手」. Appropriate when the user needs facilitated exploration rather than a direct answer. Do not activate for factual questions, direct implementation requests, or when the user has already decided and needs execution help. Boundary, so the right skill wins. This one owns the method of exploring an undecided question, not the substance of a domain that carries its own body of principles. The common overlap is structure: when what the user is undecided about is really how code, modules, or a project should be organized, yield to whichever loaded skill owns that territory, and reason from the structural question directly if none does.
