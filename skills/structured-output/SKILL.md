...
description: Principles and formatting rules for structured, precise written output, covering what to lead with, how headings carry meaning, MECE grouping, terminology consistency, specificity over hedging, and the filler patterns to strip before finishing. Consult this whenever producing writing that spans multiple sections, gets saved to a file, will be referenced later, or will be read by another agent downstream, such as reports, reviews, analyses, design docs, plans, comparisons, research summaries, handoff notes, PR descriptions, commit-adjacent writeups. English triggers include "write up a report on X", "document this", "summarize the findings", "put together a comparison", "draft a plan". 中文触发例句包括「整理成文档」「写个方案」「总结一下这次调研」「帮我出个对比」「输出一份报告」. Skip it for conversational replies and single-sentence answers, where structure is pure overhead.
---
